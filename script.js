            ...
            async function loadProductsFromDatabase() {
                try {
                    ...
                    for (const category of categories) {
                        ...
                        const products = await window.dbFunctions.getProdutosPorCategoria(category.slug);
                        ...
                        products.forEach(product => {
                                ...
                                specsHtml = `
                                    <div class="item-specs">
                                        <div class="spec-item">
                                            ...
                                            <span class="spec-value">${specs.massa || ''}</span>
                                        </div>
                                        <div class="spec-item">
                                            <span class="spec-label">Recheio</span>
                                            <span class="spec-value">${specs.recheio || ''}</span>
                                        </div>
                                        <div class="spec-item">
                                            <span class="spec-label">Finalização</span>
                                            <span class="spec-value">${specs.finalizacao || ''}</span>
                                        </div>
                                    </div>
                                `;
                                ...
                                specsHtml = `
                                    <div class="item-specs">
                                        <div class="spec-item">
                                            ...
                                            <span class="spec-value">${specs.sabores[0] || ''}</span>
                                        </div>
                                `;
                            ...
                            const productHtml = `
                                <div class="menu-item stagger-item" data-id="${product.id}" data-price="${product.preco}">
                                    <div class="item-decoration"></div>
                                    <div class="item-content">
                                        <div class="item-header">
                                            <h3 class="item-title">${product.nome}</h3>
                                            <span class="item-price">R$ ${parseFloat(product.preco).toFixed(2)}</span>
                                        </div>
                        ...
                        const staggerItems = menuItemsContainer.querySelectorAll('.stagger-item');
                        staggerItems.forEach((item, index) => {
                            setTimeout(() => {
                                item.style.transition = 'opacity 0.8s ease, transform 0.8s ease';
                                item.style.opacity = '1';
                ...
                cart.forEach(item => {
                    ...
                    cartItem.innerHTML = `
                        <div class="cart-item-info">
                            ...
                            <div class="cart-item-category">${item.section}</div>
                            <div class="cart-item-price">R$ ${item.price.toFixed(2)} un.</div>
            ...
            function updateCartTotal() {
                ...
                if (!cartTotalElement) {
                    ...
                    cartTotalDiv.innerHTML = `
                        ...
                        <div class="cart-total-amount">R$ ${totalAmount.toFixed(2)}</div>
                    `;
...
async function sendOrder() {
    ...
    try {
        ...
        // MODIFICAÇÃO AQUI: Usar URL base do GitHub
        // Obter nome do repositório a partir da URL atual
        const currentUrl = window.location.href;
        const repoBaseUrl = currentUrl.substring(0, currentUrl.lastIndexOf('/') + 1);
        ...
        alert('Pedido enviado com sucesso! Você pode acompanhar o status do seu pedido pelo link enviado.');
    } catch (error) {
